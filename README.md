...
- 🌍 **多语言支持** - 内置中英文界面，支持实时切换（[详情](./docs/I18N_QUICKSTART.md)）
- 🎯 **类型安全** - 完整的 TypeScript 类型定义
- 📋 **播放列表管理** - 完善的播放列表控制和播放模式（顺序/循环/随机/单曲）
- 🎚️ **音量控制** - 精细的音量调节 (0-100%)
- 💻 **菜单集成** - 系统托盘菜单快速访问
- 🎵 **广泛的格式支持** - 通过 FFmpeg 支持 MP3、WAV、FLAC、AAC、M4A、OGG、WMA、APE、Opus 等几乎所有音频格式（[详情](./docs/FFMPEG_GUIDE.md)）
...
- ☰ **菜单栏集成** - macOS 原生菜单栏，快速访问所有音乐控制功能
- 📝 **日志系统** - 自动记录运行日志到 `~/.haoyun-music/runtime/logs/`，便于问题排查
...
```
┌─────────────────────────────────────────────────────┐
│ 🍎 Haoyun Music Player  File  Edit  Music  View   │
├─────────────────────────────────────────────────────┤
│ Music                                               │
│  ├─ ⏯ 播放/暂停                           ⌘P       │
│  ├─ ⏮ 上一曲                              ⌘[       │
...
- [x] **菜单栏播放控制** - macOS 原生菜单栏集成，完整音乐控制
...
- [x] 托盘菜单"喜爱音乐"（按播放次数排序）
- [x] 音乐库删除功能
- [x] 系统媒体键支持（macOS/Windows，播放/暂停、上一曲、下一曲）
